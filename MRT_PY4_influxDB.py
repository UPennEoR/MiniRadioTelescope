...
while(operate):
    ...
    if not var == 'Q':
        if (var == 'M'): # Make a map!
            ...
            # Save
            print(mrtstate.directory)
            print(mrtstate.directory+time.ctime().replace(' ','_')+'.npz')
            np.savez(file=mrtstate.directory+time.ctime().replace(' ','_')+'.npz',ndata=ndata)
            # Plot
            mrtf.PlotData(ndata)
        elif (var == 'A'):
            print ("Sending "+var)
            ser.write(mrtf.AZIMUTH)
            current_state =  mrtf.StdCmd(ser,mrtf.REPORT_STATE)
            mrtf.PrintState()
        elif (var == 'L'):
            print ("Sending "+var)
            ser.write(mrtf.ELEVATION)
            current_state =  mrtf.StdCmd(ser,mrtf.REPORT_STATE)
            mrtf.PrintState()
        elif (var == 'E'):
            print ("Sending "+var)

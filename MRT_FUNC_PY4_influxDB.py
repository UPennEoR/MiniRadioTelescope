...
def parseState(buffer,state):
    ...
    if len(vars) != len(mrtstate.state_vars):
        ...
        #ndata = numpyState(state.copy()) #MAY ALSO BE TAKING UP TIME
        """ Formats data to InfluxDC readable strings and writes to InfluxDB database"""
        #for i in np.arange(len(ndata['azDeg'])):  
        #format --> identify field key + field value
        #MRTData = 'MRT,location=lab azimuth=%(az)f,elevation=%(el)f,power=%(pwr)f \n' % {'az': ndata['azDeg'], 'el': ndata['elDeg'], 'pwr': ndata['pwr']}
            #MRTData = 'MRT,location=lab azimuth=%(az)f,elevation=%(el)f,power=%(pwr)f \n' % {'az': ndata['azDeg'][i], 'el': ndata['elDeg'][i], 'pwr': ndata['pwr'][i]}
        #send string to influxdb
            #client.write_points([MRTData], protocol='line')
    return state
...
def readState(ser,init=None):
    ...
    #format --> identify field key + field value
    MRTData = 'MRT,location=lab azimuth=%(az)f,elevation=%(el)f,power=%(pwr)f \n' % {'az': ndata['azDeg'], 'el': ndata['elDeg'], 'pwr': ndata['pwr']}
    timestamp = str(int(round(time.time() * 1000000000)))
    MRTData_with_timestamp = MRTData + " " + timestamp
...
def PrintState():
    ...
    print ('Step Mode:' ,mrtstate.state['mode'][0])
    return
...
def PlotData(ndata):
    ...
    plt.savefig(mrtstate.directory+time.ctime().replace(' ','_')+'.png')
    plt.show()
    return
...
def RasterMap():
    ...
    np.savez(file=mrtstate.directory+'map_'+time.ctime().replace(' ','_')+'.npz',
...
def ScanSouthSky():
    ...
    azi = np.linspace(el.min(),el.max(),90)
    #grid the data.
    zi = griddata((az, el), pwr, (eli[None,:], azi[:,None]), method='nearest')
    #contour the gridded data
    np.savez(file=mrtstate.directory+'map_'+time.ctime().replace(' ','_')+'.npz',
